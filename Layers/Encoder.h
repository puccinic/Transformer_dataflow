...
(
	...
	hls::stream<hls::vector<T, token_length>> &gamma,
	hls::stream<hls::vector<T, token_length>> &beta,
#if defined(USING_BATCH_NORM)
	hls::stream<hls::vector<T, token_length>> &mean,
    hls::stream<hls::vector<T, token_length>> &stddev,
...
#if defined(USING_BATCH_NORM)
	batch_norm<T, sequence_length, token_length>(matadd_result1, epsilon[0], gamma, beta, mean, stddev, norm_result);
#else
	layer_norm<T, sequence_length, token_length>(matadd_result1, epsilon[0], gamma, beta, norm_result);
#endif /* using batch norm */
...
#if defined(USING_BATCH_NORM)
	batch_norm<T, sequence_length, token_length>(matadd_result2, epsilon[1], gamma, beta, mean, stddev, result);
#else
	layer_norm<T, sequence_length, token_length>(matadd_result2, epsilon[1], gamma, beta, result);
#endif /* using batch norm */
}
